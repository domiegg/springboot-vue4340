...
<div>
	<div :style='{"padding":"0","margin":"20px auto","borderColor":"#eee","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20240301/3bd53b03e3db41a8a75b2dad0cd5dacf.png) repeat-x left top / auto 8px","borderWidth":"0px","width":"80%","borderStyle":"solid","order":"3"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'☰'" :style='{"padding":"0 150px 0 120px","color":"#fff","background":"#f7c420","display":"inline-block","width":"auto","lineHeight":"50px","fontSize":"16px","justifyContent":"flex-start"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/discussjifenshangpin?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"10px 0 0","margin":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","order":"3"}'>
			...
			<div class="info" :style='{"width":"100%","padding":"0px","margin":"0","background":"none","flex":"1","order":"2"}'>
				...
				<div class="item" :style='{"padding":"2px 10px","margin":"0 0 10px 0","borderColor":"#28890b10","borderRadius":"4px","background":"#fff","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","whiteSpace":"nowrap","color":"#888","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>关联表id</div>
					<div  :style='{"padding":"8px 10px 0","wordBreak":"break-all","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.refid}}</div>
				</div>
				<div class="item" :style='{"padding":"2px 10px","margin":"0 0 10px 0","borderColor":"#28890b10","borderRadius":"4px","background":"#fff","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","whiteSpace":"nowrap","color":"#888","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>用户名</div>
					<div  :style='{"padding":"8px 10px 0","wordBreak":"break-all","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.nickname}}</div>
				</div>
				<div class="item" :style='{"padding":"2px 10px","margin":"0 0 10px 0","borderColor":"#28890b10","borderRadius":"4px","background":"#fff","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","whiteSpace":"nowrap","color":"#888","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>评论内容</div>
					<div  :style='{"padding":"8px 10px 0","wordBreak":"break-all","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.content}}</div>
				</div>
				<div class="item" :style='{"padding":"2px 10px","margin":"0 0 10px 0","borderColor":"#28890b10","borderRadius":"4px","background":"#fff","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","whiteSpace":"nowrap","color":"#888","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>回复内容</div>
					<div  :style='{"padding":"8px 10px 0","wordBreak":"break-all","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.reply}}</div>
				</div>
				<div class="btn" :style='{"width":"100%","padding":"0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px 20px 0","outline":"none","color":"#fff","borderRadius":"6px","background":"#f7c420","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('discussjifenshangpin','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px 10px 0","color":"#333","borderRadius":"6px","background":"#ffecaf","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('discussjifenshangpin','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px 10px 0","color":"#333","borderRadius":"6px","background":"#ffecaf","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('discussjifenshangpin','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
    ...
    methods: {
      ...
      sizeChange(size){
		...
		backClick(){
			...
			this.$router.push({path: '/index/discussjifenshangpin', query: params});
		},
		// 下载
		download(file){
			if(!file) {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscussjifenshangpin/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscussjifenshangpin/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscussjifenshangpin/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discussjifenshangpin/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discussjifenshangpinAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此积分商品评论表？')
			  .then(_ => {
			    this.$http.post('discussjifenshangpin/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
