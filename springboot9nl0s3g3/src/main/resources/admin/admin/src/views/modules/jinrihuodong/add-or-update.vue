<template>
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0 30px","background":"#fff"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="活动标题" prop="huodongbiaoti">
					<el-input v-model="ruleForm.huodongbiaoti" placeholder="活动标题" clearable  :readonly="ro.huodongbiaoti"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="活动标题" prop="huodongbiaoti">
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../springboot9nl0s3g3/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `jinrihuodong/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jinrihuodongCrossAddOrUpdateFlag = false;
											this.parent.search();
